...
def main(argv):
    ...
    lgd = plt.legend(loc='lower right' if FLAGS.cumulative_rmsve else 'upper right',
               frameon=True,
               prop={'size': FONTSIZE}, bbox_to_anchor=(1.5, 1), borderaxespad=0.)
    if not os.path.exists(plots):
        os.makedirs(plots)
    ...
    plt.savefig(os.path.join(plots,
                             "{}_{}.png".format("all",
                                                ...
                                                "RMSVE")), bbox_extra_artists=(lgd,), bbox_inches='tight')
...
def plot_tensorflow_log(space, color, linestyle):
    ...
    if space["crt_config"]["agent"] == "vanilla":
        ...
        plt.plot(x, mean_y_over_seeds[:99], label=format_name(space["crt_config"]["agent"],
