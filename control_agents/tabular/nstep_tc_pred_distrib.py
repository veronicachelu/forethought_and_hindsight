...
from control_agents.tabular.vanilla_tabular_control import VanillaTabularControl
...
class nStepTcPredDistrib(VanillaTabularControl):
    def __init__(
            self,
            **kwargs
    ):
        super(nStepTcPredDistrib, self).__init__(**kwargs)
    ...
    def value_update(
        ...
        for prev_o_tmn in range(np.prod(self._input_dim)):
            self._v_network[[prev_o_tmn]] = self._q_planning_opt_update(gradient *
                                                             o_tmnm1[:, prev_o_tmn],
