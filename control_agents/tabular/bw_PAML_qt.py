...
class BwPAMLQT(VanillaQT):
    def __init__(
            self,
            **kwargs
    ):
        super(BwPAMLQT, self).__init__(**kwargs)
        ...
        if self._top_n > 0:
            self._run_mode = "{}_{}".format(self._run_mode, self._top_n)
        else:
        ...
        def model_loss(q_params,
            ...
            td_errors = np.reshape(np.array(r_params[o_t] +
                                 (self._discount ** self._n) * \
                                 np.max(q_params[o_t], axis=-1) -
                                            q_params,
                                            ), (-1))
            model_Delta = P * td_errors
            real_td_error = r_t_target + (self._discount ** self._n) * \
                                         np.max(q_params[o_t], axis=-1) - q_params[o_tmn_target, a_tmn_target]
            real_Delta = np.eye(x_shape * self._nA)[oa_target_index] * real_td_error
    ...
    def model_update(
        ...
        if len(self._sequence) >= self._n:
            ...
            losses, gradients = self._model_loss_grad(self._q_network,
                                                      self._o_network,
                                                      self._r_network,
                                                      self._sequence)
