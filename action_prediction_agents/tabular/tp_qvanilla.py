...
class TpQVanilla(TabularPrediction):
    def __init__(
            self,
            run_mode: str,
            policy,
        ...
        self._a_network = model_network[2]
        self._r_network = model_network[3]
        self._d_network = model_network[4]

        def q_loss(q_params, transitions):
            o_tm1, a_tm1, r_t, d_t, o_t = transitions
            q_tm1 = q_params[o_tm1, a_tm1]
            q_t = q_params[o_t]
            q_t = np.sum(q_t * self._pi[o_t])
            q_target = r_t + d_t * discount * q_t
    ...
    def load_model(self):
        ...
        if os.path.exists(checkpoint):
            ...
            self._q_network = to_load["v_parameters"]
