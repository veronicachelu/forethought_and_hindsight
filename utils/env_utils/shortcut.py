...
class Shortcut(dm_env.Environment):
    def __init__(self, rng=None, obs_type="tabular", nS = 6, right_reward=1):
        self._P = None
        self._R = None
    ...
    def _get_next_state(self, state, action):
        if state not in self._end_states:
            if state != self._nS - 2:
                state_mask = self._rng.choice(range(2),
                                              p=[0.5, 0.5])
                next_state = state + 1 if state_mask else self._nS - 2
            else:
                next_state = state + 1
        else:
        ...
        for s in range(self._nS):
            for k in range(self._nA):
                if not (s in self._end_states):
                    ...
                    if s != self._nS - 2:
                        if fwd_s == self._nS - 2:
                            self._P[k][s][fwd_s] = 1.0
                            self._P_absorbing[k][s][fwd_s] = 1.0
                        else:
                            self._P[k][s][fwd_s] = 0.5
                            self._P[k][s][self._nS - 2] = 0.5

                            self._P_absorbing[k][s][fwd_s] = 0.5
                            self._P_absorbing[k][s][self._nS - 2] = 0.5
                    else:
                        self._P[k][s][fwd_s] = 1
