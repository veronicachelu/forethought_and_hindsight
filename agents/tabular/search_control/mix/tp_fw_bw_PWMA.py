...
class TpFwBwPWMA(TpVanilla):
    def __init__(
            self,
            **kwargs
    ):

        super(TpFwBwPWMA, self).__init__(**kwargs)
        self._sequence = []
    ...
    def planning_update(
        ...
        if self.total_steps % self._planning_period == 0:
            for k in range(self._planning_iter):
                ...
                self._add_predecessor_to_replay(o_tm1, gradient)
    ...
    def _add_predecessor_to_replay(self, o_tm1, td_error):
