...
class LpBw(LpVanilla):
    def __init__(
        ...
        def model_loss(a_params,
            ...
            # a_loss = jnp.sum(jax.vmap(rlax.l2_loss)(model_cross_o_tmn, cross_o_tmn_target))

            # model_vector_r_tmn = self._c_network(c_params,
            #                                      (lax.stop_gradient(cross_o_tmn_target),
            #                                      o_t))
            model_vector_r_tmn = self._c_network(c_params,
            ...
            # r_loss = jnp.sum(jax.vmap(rlax.l2_loss)(model_vector_r_tmn, vector_r_t_target))
            r_loss = jnp.mean(jax.vmap(rlax.l2_loss)(model_vector_r_tmn, vector_r_t_target))

            total_loss = b_loss + r_loss #a_loss

            return total_loss, {"cross_loss(A)": b_loss,
                               "expected_loss(b)": b_loss,
                               "r_loss(w_r)": r_loss
                               }

        def v_planning_grad(v_params, b_params, c_params, o_t, d_t):
            # # cross_o_tmn = self._a_forward(a_params, o_t)
            # # cross_o_tmn = lax.batch_matmul(true_o_tmn[..., None],
            # #                                jnp.transpose(true_o_tmn[..., None], axes=[0, 2, 1]))
            # expected_o_tmn = self._b_forward(b_params, o_t)
            # cross_o_tmn = lax.batch_matmul(expected_o_tmn[..., None],
            #                                jnp.transpose(expected_o_tmn[..., None], axes=[0, 2, 1]))
            # # expected_o_tmn = true_o_tmn
            # r_input = jnp.concatenate([expected_o_tmn, o_t], axis=-1)
            #
            # r_tmn = self._c_forward(c_params, r_input)
            # # vector_r_tmn = self._c_network(c_params, (cross_o_tmn, o_t))[..., None]
            # # vector_r_tmn = self._c_network(c_params, (expected_o_tmn, o_t))[..., None]
            # # vector_r_tmn = (expected_o_tmn *
            # #                 self._c_network(c_params, (expected_o_tmn, o_t)))[..., None]
            # vector_r_tmn = (expected_o_tmn * r_tmn)[..., None]
            ...
            def td_fun(v_params, b_params, c_params, o_t, d_t):
                o_tmn = self._b_forward(b_params, o_t)
                ...
                td_error = jax.vmap(rlax.td_learning)(v_tmn, r_tmn,
                                                      ...
                                                      v_t_target)
                _, vjp_fun = jax.vjp(self._v_network, v_params, o_tmn)

                grad = -vjp_fun(2 * td_error[..., None])[0]
                return jnp.mean(td_error ** 2), grad
            # _, vjp_fun = jax.vjp(self._v_network, v_params, o_tmn)
            #
            # return vjp_fun(td_error[..., None])[0]
            grad_fn = jax.grad(td_fun, 0, has_aux=True)
            grad, grad2 = grad_fn(v_params, b_params, c_params, o_t, d_t)
        ...
        # self._v_planning_loss_grad = jax.jit(jax.grad(v_planning_grad, 0))
        ...
        self._model_opt_state = model_opt_init([self._b_parameters, self._c_parameters])
    ...
    def model_update(
        ...
        if len(self._sequence) >= self._n:
            ...
            self._b_parameters, self._c_parameters = self._model_parameters
