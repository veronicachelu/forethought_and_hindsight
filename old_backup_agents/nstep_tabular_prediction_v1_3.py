...
class nStepTabularPredictionV13(VanillaTabularPrediction):
    def __init__(
            self,
            **kwargs
    ):
        super(nStepTabularPredictionV13, self).__init__(**kwargs)

        self._sequence = []
        self._offline_updates = []
        self._reset_offline_updates = False
    ...
    def planning_update(self,
        ...
        if self._reset_offline_updates:
            for o_t, delta_t in self._offline_updates:
                for prev_o_tmn in range(np.prod(self._input_dim)):
                    self._v_network[prev_o_tmn] = self._planning_opt_update(delta_t *
                                                                     (self._discount ** self._n) *
                                                                     self._o_network[o_t, prev_o_tmn],
                                                                     self._v_network[prev_o_tmn])
            self._offline_updates.clear()
    ...
    def value_update(
        ...
        self._offline_updates.append((o_tm1, gradient))
        if new_timestep.discount == 0:
            self._reset_offline_updates = True
        # self._v_network[o_tm1] = self._v_opt_update(gradient, self._v_network[o_tm1])
        # for prev_o_tmn in range(np.prod(self._input_dim)):
        #     self._v_network[prev_o_tmn] = self._planning_opt_update(gradient *
        #                                                      (self._discount ** self._n) *
        #                                                      self._o_network[o_tm1, prev_o_tmn],
        #                                                      self._v_network[prev_o_tmn])
