...
class nStepLinearPredictionV12(VanillaLinearPrediction):
    def __init__(
            self,
            **kwargs
    ):
        super(nStepLinearPredictionV12, self).__init__(**kwargs)
    ...
    def value_update(
        ...
        self._update_predecessors(np.array([timestep.observation]), 2 * td_error, 0)
    ...
    def _update_predecessors(self, o_t, td_error, horizon):
        o_tmn = self._o_forward(self._o_parameters, o_t)
        ...
        y, vjp_fun = jax.vjp(self._v_network, self._v_parameters, o_tmn)
        discounted_td_error = td_error * (self._discount ** self._n)
        predecessor_gradient = vjp_fun(discounted_td_error)[0]
        self._v_opt_state = self._v_opt_update(self.total_steps, predecessor_gradient,
                                               self._v_opt_state)
        self._v_parameters = self._v_get_params(self._v_opt_state)
        horizon += self._n
        if horizon < 1 / (1 - self._discount):
            self._update_predecessors(o_tmn, discounted_td_error, horizon)
