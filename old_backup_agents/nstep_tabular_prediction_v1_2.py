...
class nStepTabularPredictionV12(VanillaTabularPrediction):
    def __init__(
            self,
            **kwargs
    ):
        super(nStepTabularPredictionV12, self).__init__(**kwargs)
    ...
    def value_update(
        ...
        self._update_predecessors(o_tm1, gradient)

        losses_and_grads = {"losses": {"loss_v": np.array(loss)},
                            }
        self._log_summaries(losses_and_grads, "value")

    def _update_predecessors(self, o_t, gradient):
        o_tmn = self._o_network[o_t]
        divisior = np.sum(o_tmn, axis=-1, keepdims=True)
        o_tmn = np.divide(o_tmn, divisior, out=np.zeros_like(o_tmn), where=np.all(divisior != 0))
        for prev_o_tmn in range(np.prod(self._input_dim)):
            p_o_tmn = o_tmn[:, prev_o_tmn]
            predecessor_gradient = gradient * (self._discount ** self._n)
            self._v_network[[prev_o_tmn]] = self._v_opt_update(predecessor_gradient * p_o_tmn,
                                                             self._v_network[[prev_o_tmn]])
            if np.all(p_o_tmn != 0):
                self._update_predecessors([prev_o_tmn], predecessor_gradient)
